...
const { chmodSync, createWriteStream, mkdirSync, readFileSync, writeFileSync } = require('fs')
         ...
         default: {
            ...
            for await (const entry of zip) {
               ...
               if (fileName === `${base}/node.exe`) {
                  entry.pipe(createWriteStream(join(assetDir, 'node.exe')))
               } else {
                  entry.autodrain()
               }
            }
